        ...
        !chat.hideInChat && (
            <div className={`massage ${chat.role === "model" ? "bot" : "user"}-massage ${chat.isError ? "error" : ""} flex gap-3 flex-col`}>
                {chat.role === "model" && (
                    ...
                    <img src={roboticon} alt="" className="text-xl p-2 fill-[#fff] bg-[#1570bf] rounded-full w-10 h-10" />
                    
                )}
                <p className="massage-text px-4 py-3 max-w-[75%] text-[15px] bg-gray-700 text-[#fff]">
                    {chat.text}
                </p>
            </div>
        )
